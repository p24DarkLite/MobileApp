...
export default function LoginScreen({ navigation }) {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1, backgroundColor: isDark ? '#000' : '#fff' }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
        <View style={styles.innerContainer}>
          ...
          <View style={styles.inputGroup}>
            <TextInput style={[styles.input, { borderColor: emailError ? '#ff4444' : inputBorder, color: textColor, backgroundColor: inputBg }]} placeholder={t.email} placeholderTextColor="#666" value={email} onChangeText={(t) => { setEmail(t); setEmailError(''); }} autoCapitalize="none" />
            {emailError ? <Text style={styles.errorText}>{emailError}</Text> : null}
          </View>
          <View style={styles.inputGroup}>
            <TextInput style={[styles.input, { borderColor: passwordError ? '#ff4444' : inputBorder, color: textColor, backgroundColor: inputBg }]} placeholder={t.password} secureTextEntry placeholderTextColor="#666" value={password} onChangeText={(t) => { setPassword(t); setPasswordError(''); }} />
